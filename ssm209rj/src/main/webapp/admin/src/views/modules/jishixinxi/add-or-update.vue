    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="个人特长" prop="gerentezhang">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.gerentezhang" label="个人特长" prop="gerentezhang">
                    <span v-html="ruleForm.gerentezhang"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	userid : false,
      ...
      rules: {
          jishizhanghao: [
                { required: true, message: '技师账号不能为空', trigger: 'blur' },
          ],
          jishixingming: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.jishizhanghao!=''&&json.jishizhanghao){
              		this.ruleForm.jishizhanghao = json.jishizhanghao
		}
		...
		if(json.gongling!=''&&json.gongling){
              		this.ruleForm.gongling = json.gongling
		}
		...
		if(json.zhaopian!=''&&json.zhaopian){
          ...
          this.$message.error(data.msg);
            ...
            this.dangqianzhuangtaiOptions = "空闲,忙碌中".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `jishixinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.gongzuojingli = this.ruleForm.gongzuojingli.replace(reg,'../../../ssm209rj/upload');
	this.ruleForm.gerentezhang = this.ruleForm.gerentezhang.replace(reg,'../../../ssm209rj/upload');
	this.ruleForm.xiangxixinxi = this.ruleForm.xiangxixinxi.replace(reg,'../../../ssm209rj/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jishixinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jishixinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
